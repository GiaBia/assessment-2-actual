...
// CODE HERE
const canWeDeliverTwo = (postalCode) => {
    if (deliveryAreaZipCodes.includes(postalCode) === true) {
        return `You're in our delivery zone!`
    } else {
        return `Sorry, we can't deliver to that address.`
    }
...
function replaceObjectParam(deals, key, old_value, new_value) {
    return deals.map((obj) => {
...
newDeals = replaceObjectParam(deals, "title", "15%", "10%")
